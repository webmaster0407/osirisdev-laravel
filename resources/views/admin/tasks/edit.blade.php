        ...
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('createduser') ? 'is-invalid' : '' }}" name="createduser_id" id="createduser_id">
                    @foreach($createdusers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('createduser_id') ? old('createduser_id') : $task->createduser->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assigneduser') ? 'is-invalid' : '' }}" name="assigneduser_id" id="assigneduser_id">
                    @foreach($assignedusers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('assigneduser_id') ? old('assigneduser_id') : $task->assigneduser->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('completed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="completed" id="completed" value="1" {{ $task->completed || old('completed', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('relationtype') ? 'is-invalid' : '' }}" type="text" name="relationtype" id="relationtype" value="{{ old('relationtype', $task->relationtype) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('relationid') ? 'is-invalid' : '' }}" type="number" name="relationid" id="relationid" value="{{ old('relationid', $task->relationid) }}" step="1">
