...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.prevregistration.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.prevregistrations.update", [$prevregistration->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $prevregistration->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prev') ? 'is-invalid' : '' }}" name="prev_id" id="prev_id" required>
                    @foreach($prevs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('prev_id') ? old('prev_id') : $prevregistration->prev->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('prev'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('regnotes') ? 'is-invalid' : '' }}" type="text" name="regnotes" id="regnotes" value="{{ old('regnotes', $prevregistration->regnotes) }}">
