    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.notes.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" type="text" name="note" id="note" value="{{ old('note', '') }}" required>
                @if($errors->has('note'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('relationtype') ? 'is-invalid' : '' }}" type="text" name="relationtype" id="relationtype" value="{{ old('relationtype', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('relationid') ? 'is-invalid' : '' }}" type="number" name="relationid" id="relationid" value="{{ old('relationid', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('visability') ? 'is-invalid' : '' }}" name="visability" id="visability">
                    ...
                    @foreach(App\Models\Note::VISABILITY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('visability', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
