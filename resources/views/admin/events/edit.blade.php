    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Event::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $event->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('internalinfo') ? 'is-invalid' : '' }}" name="internalinfo" id="internalinfo">{!! old('internalinfo', $event->internalinfo) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('starttime') ? 'is-invalid' : '' }}" type="text" name="starttime" id="starttime" value="{{ old('starttime', $event->starttime) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('endtime') ? 'is-invalid' : '' }}" type="text" name="endtime" id="endtime" value="{{ old('endtime', $event->endtime) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}" name="location_id" id="location_id">
                    @foreach($locations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('location_id') ? old('location_id') : $event->location->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('created_by') ? 'is-invalid' : '' }}" name="created_by_id" id="created_by_id" required>
                    @foreach($created_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('created_by_id') ? old('created_by_id') : $event->created_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
