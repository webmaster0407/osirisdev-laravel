    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.competences.update", [$competence->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('key') ? 'is-invalid' : '' }}" type="text" name="key" id="key" value="{{ old('key', $competence->key) }}" required>
                @if($errors->has('key'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Competence::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $competence->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('color') ? 'is-invalid' : '' }}" type="text" name="color" id="color" value="{{ old('color', $competence->color) }}" required>
                @if($errors->has('color'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('expirable') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="expirable" id="expirable" value="1" {{ $competence->expirable || old('expirable', 0) === 1 ? 'checked' : '' }}>
